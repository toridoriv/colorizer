import _1c from "@shikijs/langs/1c";
import _1cQuery from "@shikijs/langs/1c-query";
import abap from "@shikijs/langs/abap";
import actionscript3 from "@shikijs/langs/actionscript-3";
import ada from "@shikijs/langs/ada";
import adoc from "@shikijs/langs/adoc";
import angularExpression from "@shikijs/langs/angular-expression";
import angularHtml from "@shikijs/langs/angular-html";
import angularInlineStyle from "@shikijs/langs/angular-inline-style";
import angularInlineTemplate from "@shikijs/langs/angular-inline-template";
import angularLetDeclaration from "@shikijs/langs/angular-let-declaration";
import angularTemplate from "@shikijs/langs/angular-template";
import angularTemplateBlocks from "@shikijs/langs/angular-template-blocks";
import angularTs from "@shikijs/langs/angular-ts";
import apache from "@shikijs/langs/apache";
import apex from "@shikijs/langs/apex";
import apl from "@shikijs/langs/apl";
import applescript from "@shikijs/langs/applescript";
import ara from "@shikijs/langs/ara";
import asciidoc from "@shikijs/langs/asciidoc";
import asm from "@shikijs/langs/asm";
import astro from "@shikijs/langs/astro";
import awk from "@shikijs/langs/awk";
import ballerina from "@shikijs/langs/ballerina";
import bash from "@shikijs/langs/bash";
import bat from "@shikijs/langs/bat";
import batch from "@shikijs/langs/batch";
import be from "@shikijs/langs/be";
import beancount from "@shikijs/langs/beancount";
import berry from "@shikijs/langs/berry";
import bibtex from "@shikijs/langs/bibtex";
import bicep from "@shikijs/langs/bicep";
import blade from "@shikijs/langs/blade";
import bsl from "@shikijs/langs/bsl";
import c from "@shikijs/langs/c";
import cadence from "@shikijs/langs/cadence";
import cairo from "@shikijs/langs/cairo";
import cdc from "@shikijs/langs/cdc";
import cjs from "@shikijs/langs/cjs";
import clarity from "@shikijs/langs/clarity";
import clj from "@shikijs/langs/clj";
import clojure from "@shikijs/langs/clojure";
import closureTemplates from "@shikijs/langs/closure-templates";
import cmake from "@shikijs/langs/cmake";
import cmd from "@shikijs/langs/cmd";
import cobol from "@shikijs/langs/cobol";
import codeowners from "@shikijs/langs/codeowners";
import codeql from "@shikijs/langs/codeql";
import coffee from "@shikijs/langs/coffee";
import coffeescript from "@shikijs/langs/coffeescript";
import commonLisp from "@shikijs/langs/common-lisp";
import console from "@shikijs/langs/console";
import coq from "@shikijs/langs/coq";
import cpp from "@shikijs/langs/cpp";
import cppMacro from "@shikijs/langs/cpp-macro";
import cql from "@shikijs/langs/cql";
import crystal from "@shikijs/langs/crystal";
import cs from "@shikijs/langs/cs";
import csharp from "@shikijs/langs/csharp";
import css from "@shikijs/langs/css";
import csv from "@shikijs/langs/csv";
import cts from "@shikijs/langs/cts";
import cue from "@shikijs/langs/cue";
import cypher from "@shikijs/langs/cypher";
import d from "@shikijs/langs/d";
import dart from "@shikijs/langs/dart";
import dax from "@shikijs/langs/dax";
import desktop from "@shikijs/langs/desktop";
import diff from "@shikijs/langs/diff";
import docker from "@shikijs/langs/docker";
import dockerfile from "@shikijs/langs/dockerfile";
import dotenv from "@shikijs/langs/dotenv";
import dreamMaker from "@shikijs/langs/dream-maker";
import edge from "@shikijs/langs/edge";
import elisp from "@shikijs/langs/elisp";
import elixir from "@shikijs/langs/elixir";
import elm from "@shikijs/langs/elm";
import emacsLisp from "@shikijs/langs/emacs-lisp";
import erb from "@shikijs/langs/erb";
import erl from "@shikijs/langs/erl";
import erlang from "@shikijs/langs/erlang";
import esTagCss from "@shikijs/langs/es-tag-css";
import esTagGlsl from "@shikijs/langs/es-tag-glsl";
import esTagHtml from "@shikijs/langs/es-tag-html";
import esTagSql from "@shikijs/langs/es-tag-sql";
import esTagXml from "@shikijs/langs/es-tag-xml";
import f from "@shikijs/langs/f";
import f03 from "@shikijs/langs/f03";
import f08 from "@shikijs/langs/f08";
import f18 from "@shikijs/langs/f18";
import f77 from "@shikijs/langs/f77";
import f90 from "@shikijs/langs/f90";
import f95 from "@shikijs/langs/f95";
import fennel from "@shikijs/langs/fennel";
import fish from "@shikijs/langs/fish";
import fluent from "@shikijs/langs/fluent";
import _for from "@shikijs/langs/for";
import fortranFixedForm from "@shikijs/langs/fortran-fixed-form";
import fortranFreeForm from "@shikijs/langs/fortran-free-form";
import fs from "@shikijs/langs/fs";
import fsharp from "@shikijs/langs/fsharp";
import fsl from "@shikijs/langs/fsl";
import ftl from "@shikijs/langs/ftl";
import gdresource from "@shikijs/langs/gdresource";
import gdscript from "@shikijs/langs/gdscript";
import gdshader from "@shikijs/langs/gdshader";
import genie from "@shikijs/langs/genie";
import gherkin from "@shikijs/langs/gherkin";
import gitCommit from "@shikijs/langs/git-commit";
import gitRebase from "@shikijs/langs/git-rebase";
import gjs from "@shikijs/langs/gjs";
import gleam from "@shikijs/langs/gleam";
import glimmerJs from "@shikijs/langs/glimmer-js";
import glimmerTs from "@shikijs/langs/glimmer-ts";
import glsl from "@shikijs/langs/glsl";
import gnuplot from "@shikijs/langs/gnuplot";
import go from "@shikijs/langs/go";
import gql from "@shikijs/langs/gql";
import graphql from "@shikijs/langs/graphql";
import groovy from "@shikijs/langs/groovy";
import gts from "@shikijs/langs/gts";
import hack from "@shikijs/langs/hack";
import haml from "@shikijs/langs/haml";
import handlebars from "@shikijs/langs/handlebars";
import haskell from "@shikijs/langs/haskell";
import haxe from "@shikijs/langs/haxe";
import hbs from "@shikijs/langs/hbs";
import hcl from "@shikijs/langs/hcl";
import hjson from "@shikijs/langs/hjson";
import hlsl from "@shikijs/langs/hlsl";
import hs from "@shikijs/langs/hs";
import html from "@shikijs/langs/html";
import htmlDerivative from "@shikijs/langs/html-derivative";
import http from "@shikijs/langs/http";
import hurl from "@shikijs/langs/hurl";
import hxml from "@shikijs/langs/hxml";
import hy from "@shikijs/langs/hy";
import imba from "@shikijs/langs/imba";
import ini from "@shikijs/langs/ini";
import jade from "@shikijs/langs/jade";
import java from "@shikijs/langs/java";
import javascript from "@shikijs/langs/javascript";
import jinja from "@shikijs/langs/jinja";
import jinjaHtml from "@shikijs/langs/jinja-html";
import jison from "@shikijs/langs/jison";
import jl from "@shikijs/langs/jl";
import js from "@shikijs/langs/js";
import json from "@shikijs/langs/json";
import json5 from "@shikijs/langs/json5";
import jsonc from "@shikijs/langs/jsonc";
import jsonl from "@shikijs/langs/jsonl";
import jsonnet from "@shikijs/langs/jsonnet";
import jssm from "@shikijs/langs/jssm";
import jsx from "@shikijs/langs/jsx";
import julia from "@shikijs/langs/julia";
import kdl from "@shikijs/langs/kdl";
import kotlin from "@shikijs/langs/kotlin";
import kql from "@shikijs/langs/kql";
import kt from "@shikijs/langs/kt";
import kts from "@shikijs/langs/kts";
import kusto from "@shikijs/langs/kusto";
import latex from "@shikijs/langs/latex";
import lean from "@shikijs/langs/lean";
import lean4 from "@shikijs/langs/lean4";
import less from "@shikijs/langs/less";
import liquid from "@shikijs/langs/liquid";
import lisp from "@shikijs/langs/lisp";
import lit from "@shikijs/langs/lit";
import llvm from "@shikijs/langs/llvm";
import log from "@shikijs/langs/log";
import logo from "@shikijs/langs/logo";
import lua from "@shikijs/langs/lua";
import luau from "@shikijs/langs/luau";
import make from "@shikijs/langs/make";
import makefile from "@shikijs/langs/makefile";
import markdown from "@shikijs/langs/markdown";
import markdownVue from "@shikijs/langs/markdown-vue";
import marko from "@shikijs/langs/marko";
import matlab from "@shikijs/langs/matlab";
import md from "@shikijs/langs/md";
import mdc from "@shikijs/langs/mdc";
import mdx from "@shikijs/langs/mdx";
import mediawiki from "@shikijs/langs/mediawiki";
import mermaid from "@shikijs/langs/mermaid";
import mips from "@shikijs/langs/mips";
import mipsasm from "@shikijs/langs/mipsasm";
import mjs from "@shikijs/langs/mjs";
import mmd from "@shikijs/langs/mmd";
import mojo from "@shikijs/langs/mojo";
import move from "@shikijs/langs/move";
import mts from "@shikijs/langs/mts";
import nar from "@shikijs/langs/nar";
import narrat from "@shikijs/langs/narrat";
import nextflow from "@shikijs/langs/nextflow";
import nf from "@shikijs/langs/nf";
import nginx from "@shikijs/langs/nginx";
import nim from "@shikijs/langs/nim";
import nix from "@shikijs/langs/nix";
import nu from "@shikijs/langs/nu";
import nushell from "@shikijs/langs/nushell";
import objc from "@shikijs/langs/objc";
import objectiveC from "@shikijs/langs/objective-c";
import objectiveCpp from "@shikijs/langs/objective-cpp";
import ocaml from "@shikijs/langs/ocaml";
import pascal from "@shikijs/langs/pascal";
import perl from "@shikijs/langs/perl";
import perl6 from "@shikijs/langs/perl6";
import php from "@shikijs/langs/php";
import pkl from "@shikijs/langs/pkl";
import plsql from "@shikijs/langs/plsql";
import po from "@shikijs/langs/po";
import polar from "@shikijs/langs/polar";
import postcss from "@shikijs/langs/postcss";
import pot from "@shikijs/langs/pot";
import potx from "@shikijs/langs/potx";
import powerquery from "@shikijs/langs/powerquery";
import powershell from "@shikijs/langs/powershell";
import prisma from "@shikijs/langs/prisma";
import prolog from "@shikijs/langs/prolog";
import properties from "@shikijs/langs/properties";
import proto from "@shikijs/langs/proto";
import protobuf from "@shikijs/langs/protobuf";
import ps from "@shikijs/langs/ps";
import ps1 from "@shikijs/langs/ps1";
import pug from "@shikijs/langs/pug";
import puppet from "@shikijs/langs/puppet";
import purescript from "@shikijs/langs/purescript";
import py from "@shikijs/langs/py";
import python from "@shikijs/langs/python";
import ql from "@shikijs/langs/ql";
import qml from "@shikijs/langs/qml";
import qmldir from "@shikijs/langs/qmldir";
import qss from "@shikijs/langs/qss";
import r from "@shikijs/langs/r";
import racket from "@shikijs/langs/racket";
import raku from "@shikijs/langs/raku";
import razor from "@shikijs/langs/razor";
import rb from "@shikijs/langs/rb";
import reg from "@shikijs/langs/reg";
import regex from "@shikijs/langs/regex";
import regexp from "@shikijs/langs/regexp";
import rel from "@shikijs/langs/rel";
import riscv from "@shikijs/langs/riscv";
import rosmsg from "@shikijs/langs/rosmsg";
import rs from "@shikijs/langs/rs";
import rst from "@shikijs/langs/rst";
import ruby from "@shikijs/langs/ruby";
import rust from "@shikijs/langs/rust";
import sas from "@shikijs/langs/sas";
import sass from "@shikijs/langs/sass";
import scala from "@shikijs/langs/scala";
import scheme from "@shikijs/langs/scheme";
import scss from "@shikijs/langs/scss";
import sdbl from "@shikijs/langs/sdbl";
import sh from "@shikijs/langs/sh";
import shader from "@shikijs/langs/shader";
import shaderlab from "@shikijs/langs/shaderlab";
import shell from "@shikijs/langs/shell";
import shellscript from "@shikijs/langs/shellscript";
import shellsession from "@shikijs/langs/shellsession";
import smalltalk from "@shikijs/langs/smalltalk";
import solidity from "@shikijs/langs/solidity";
import soy from "@shikijs/langs/soy";
import sparql from "@shikijs/langs/sparql";
import spl from "@shikijs/langs/spl";
import splunk from "@shikijs/langs/splunk";
import sql from "@shikijs/langs/sql";
import sshConfig from "@shikijs/langs/ssh-config";
import stata from "@shikijs/langs/stata";
import styl from "@shikijs/langs/styl";
import stylus from "@shikijs/langs/stylus";
import svelte from "@shikijs/langs/svelte";
import swift from "@shikijs/langs/swift";
import systemVerilog from "@shikijs/langs/system-verilog";
import systemd from "@shikijs/langs/systemd";
import talon from "@shikijs/langs/talon";
import talonscript from "@shikijs/langs/talonscript";
import tasl from "@shikijs/langs/tasl";
import tcl from "@shikijs/langs/tcl";
import templ from "@shikijs/langs/templ";
import terraform from "@shikijs/langs/terraform";
import tex from "@shikijs/langs/tex";
import tf from "@shikijs/langs/tf";
import tfvars from "@shikijs/langs/tfvars";
import toml from "@shikijs/langs/toml";
import ts from "@shikijs/langs/ts";
import tsTags from "@shikijs/langs/ts-tags";
import tsp from "@shikijs/langs/tsp";
import tsv from "@shikijs/langs/tsv";
import tsx from "@shikijs/langs/tsx";
import turtle from "@shikijs/langs/turtle";
import twig from "@shikijs/langs/twig";
import typ from "@shikijs/langs/typ";
import typescript from "@shikijs/langs/typescript";
import typespec from "@shikijs/langs/typespec";
import typst from "@shikijs/langs/typst";
import v from "@shikijs/langs/v";
import vala from "@shikijs/langs/vala";
import vb from "@shikijs/langs/vb";
import verilog from "@shikijs/langs/verilog";
import vhdl from "@shikijs/langs/vhdl";
import vim from "@shikijs/langs/vim";
import viml from "@shikijs/langs/viml";
import vimscript from "@shikijs/langs/vimscript";
import vue from "@shikijs/langs/vue";
import vueDirectives from "@shikijs/langs/vue-directives";
import vueHtml from "@shikijs/langs/vue-html";
import vueInterpolations from "@shikijs/langs/vue-interpolations";
import vueSfcStyleVariableInjection from "@shikijs/langs/vue-sfc-style-variable-injection";
import vueVine from "@shikijs/langs/vue-vine";
import vy from "@shikijs/langs/vy";
import vyper from "@shikijs/langs/vyper";
import wasm from "@shikijs/langs/wasm";
import wenyan from "@shikijs/langs/wenyan";
import wgsl from "@shikijs/langs/wgsl";
import wiki from "@shikijs/langs/wiki";
import wikitext from "@shikijs/langs/wikitext";
import wit from "@shikijs/langs/wit";
import wl from "@shikijs/langs/wl";
import wolfram from "@shikijs/langs/wolfram";
import xml from "@shikijs/langs/xml";
import xsl from "@shikijs/langs/xsl";
import yaml from "@shikijs/langs/yaml";
import yml from "@shikijs/langs/yml";
import zenscript from "@shikijs/langs/zenscript";
import zig from "@shikijs/langs/zig";
import zsh from "@shikijs/langs/zsh";

export type Language =
  | "1c"
  | "1c-query"
  | "abap"
  | "actionscript-3"
  | "ada"
  | "adoc"
  | "angular-expression"
  | "angular-html"
  | "angular-inline-style"
  | "angular-inline-template"
  | "angular-let-declaration"
  | "angular-template"
  | "angular-template-blocks"
  | "angular-ts"
  | "apache"
  | "apex"
  | "apl"
  | "applescript"
  | "ara"
  | "asciidoc"
  | "asm"
  | "astro"
  | "awk"
  | "ballerina"
  | "bash"
  | "bat"
  | "batch"
  | "be"
  | "beancount"
  | "berry"
  | "bibtex"
  | "bicep"
  | "blade"
  | "bsl"
  | "c"
  | "cadence"
  | "cairo"
  | "cdc"
  | "cjs"
  | "clarity"
  | "clj"
  | "clojure"
  | "closure-templates"
  | "cmake"
  | "cmd"
  | "cobol"
  | "codeowners"
  | "codeql"
  | "coffee"
  | "coffeescript"
  | "common-lisp"
  | "console"
  | "coq"
  | "cpp"
  | "cpp-macro"
  | "cql"
  | "crystal"
  | "cs"
  | "csharp"
  | "css"
  | "csv"
  | "cts"
  | "cue"
  | "cypher"
  | "d"
  | "dart"
  | "dax"
  | "desktop"
  | "diff"
  | "docker"
  | "dockerfile"
  | "dotenv"
  | "dream-maker"
  | "edge"
  | "elisp"
  | "elixir"
  | "elm"
  | "emacs-lisp"
  | "erb"
  | "erl"
  | "erlang"
  | "es-tag-css"
  | "es-tag-glsl"
  | "es-tag-html"
  | "es-tag-sql"
  | "es-tag-xml"
  | "f"
  | "f03"
  | "f08"
  | "f18"
  | "f77"
  | "f90"
  | "f95"
  | "fennel"
  | "fish"
  | "fluent"
  | "for"
  | "fortran-fixed-form"
  | "fortran-free-form"
  | "fs"
  | "fsharp"
  | "fsl"
  | "ftl"
  | "gdresource"
  | "gdscript"
  | "gdshader"
  | "genie"
  | "gherkin"
  | "git-commit"
  | "git-rebase"
  | "gjs"
  | "gleam"
  | "glimmer-js"
  | "glimmer-ts"
  | "glsl"
  | "gnuplot"
  | "go"
  | "gql"
  | "graphql"
  | "groovy"
  | "gts"
  | "hack"
  | "haml"
  | "handlebars"
  | "haskell"
  | "haxe"
  | "hbs"
  | "hcl"
  | "hjson"
  | "hlsl"
  | "hs"
  | "html"
  | "html-derivative"
  | "http"
  | "hurl"
  | "hxml"
  | "hy"
  | "imba"
  | "ini"
  | "jade"
  | "java"
  | "javascript"
  | "jinja"
  | "jinja-html"
  | "jison"
  | "jl"
  | "js"
  | "json"
  | "json5"
  | "jsonc"
  | "jsonl"
  | "jsonnet"
  | "jssm"
  | "jsx"
  | "julia"
  | "kdl"
  | "kotlin"
  | "kql"
  | "kt"
  | "kts"
  | "kusto"
  | "latex"
  | "lean"
  | "lean4"
  | "less"
  | "liquid"
  | "lisp"
  | "lit"
  | "llvm"
  | "log"
  | "logo"
  | "lua"
  | "luau"
  | "make"
  | "makefile"
  | "markdown"
  | "markdown-vue"
  | "marko"
  | "matlab"
  | "md"
  | "mdc"
  | "mdx"
  | "mediawiki"
  | "mermaid"
  | "mips"
  | "mipsasm"
  | "mjs"
  | "mmd"
  | "mojo"
  | "move"
  | "mts"
  | "nar"
  | "narrat"
  | "nextflow"
  | "nf"
  | "nginx"
  | "nim"
  | "nix"
  | "nu"
  | "nushell"
  | "objc"
  | "objective-c"
  | "objective-cpp"
  | "ocaml"
  | "pascal"
  | "perl"
  | "perl6"
  | "php"
  | "pkl"
  | "plsql"
  | "po"
  | "polar"
  | "postcss"
  | "pot"
  | "potx"
  | "powerquery"
  | "powershell"
  | "prisma"
  | "prolog"
  | "properties"
  | "proto"
  | "protobuf"
  | "ps"
  | "ps1"
  | "pug"
  | "puppet"
  | "purescript"
  | "py"
  | "python"
  | "ql"
  | "qml"
  | "qmldir"
  | "qss"
  | "r"
  | "racket"
  | "raku"
  | "razor"
  | "rb"
  | "reg"
  | "regex"
  | "regexp"
  | "rel"
  | "riscv"
  | "rosmsg"
  | "rs"
  | "rst"
  | "ruby"
  | "rust"
  | "sas"
  | "sass"
  | "scala"
  | "scheme"
  | "scss"
  | "sdbl"
  | "sh"
  | "shader"
  | "shaderlab"
  | "shell"
  | "shellscript"
  | "shellsession"
  | "smalltalk"
  | "solidity"
  | "soy"
  | "sparql"
  | "spl"
  | "splunk"
  | "sql"
  | "ssh-config"
  | "stata"
  | "styl"
  | "stylus"
  | "svelte"
  | "swift"
  | "system-verilog"
  | "systemd"
  | "talon"
  | "talonscript"
  | "tasl"
  | "tcl"
  | "templ"
  | "terraform"
  | "tex"
  | "tf"
  | "tfvars"
  | "toml"
  | "ts"
  | "ts-tags"
  | "tsp"
  | "tsv"
  | "tsx"
  | "turtle"
  | "twig"
  | "typ"
  | "typescript"
  | "typespec"
  | "typst"
  | "v"
  | "vala"
  | "vb"
  | "verilog"
  | "vhdl"
  | "vim"
  | "viml"
  | "vimscript"
  | "vue"
  | "vue-directives"
  | "vue-html"
  | "vue-interpolations"
  | "vue-sfc-style-variable-injection"
  | "vue-vine"
  | "vy"
  | "vyper"
  | "wasm"
  | "wenyan"
  | "wgsl"
  | "wiki"
  | "wikitext"
  | "wit"
  | "wl"
  | "wolfram"
  | "xml"
  | "xsl"
  | "yaml"
  | "yml"
  | "zenscript"
  | "zig"
  | "zsh";

export const languages = [
  _1c,
  _1cQuery,
  abap,
  actionscript3,
  ada,
  adoc,
  angularExpression,
  angularHtml,
  angularInlineStyle,
  angularInlineTemplate,
  angularLetDeclaration,
  angularTemplate,
  angularTemplateBlocks,
  angularTs,
  apache,
  apex,
  apl,
  applescript,
  ara,
  asciidoc,
  asm,
  astro,
  awk,
  ballerina,
  bash,
  bat,
  batch,
  be,
  beancount,
  berry,
  bibtex,
  bicep,
  blade,
  bsl,
  c,
  cadence,
  cairo,
  cdc,
  cjs,
  clarity,
  clj,
  clojure,
  closureTemplates,
  cmake,
  cmd,
  cobol,
  codeowners,
  codeql,
  coffee,
  coffeescript,
  commonLisp,
  console,
  coq,
  cpp,
  cppMacro,
  cql,
  crystal,
  cs,
  csharp,
  css,
  csv,
  cts,
  cue,
  cypher,
  d,
  dart,
  dax,
  desktop,
  diff,
  docker,
  dockerfile,
  dotenv,
  dreamMaker,
  edge,
  elisp,
  elixir,
  elm,
  emacsLisp,
  erb,
  erl,
  erlang,
  esTagCss,
  esTagGlsl,
  esTagHtml,
  esTagSql,
  esTagXml,
  f,
  f03,
  f08,
  f18,
  f77,
  f90,
  f95,
  fennel,
  fish,
  fluent,
  _for,
  fortranFixedForm,
  fortranFreeForm,
  fs,
  fsharp,
  fsl,
  ftl,
  gdresource,
  gdscript,
  gdshader,
  genie,
  gherkin,
  gitCommit,
  gitRebase,
  gjs,
  gleam,
  glimmerJs,
  glimmerTs,
  glsl,
  gnuplot,
  go,
  gql,
  graphql,
  groovy,
  gts,
  hack,
  haml,
  handlebars,
  haskell,
  haxe,
  hbs,
  hcl,
  hjson,
  hlsl,
  hs,
  html,
  htmlDerivative,
  http,
  hurl,
  hxml,
  hy,
  imba,
  ini,
  jade,
  java,
  javascript,
  jinja,
  jinjaHtml,
  jison,
  jl,
  js,
  json,
  json5,
  jsonc,
  jsonl,
  jsonnet,
  jssm,
  jsx,
  julia,
  kdl,
  kotlin,
  kql,
  kt,
  kts,
  kusto,
  latex,
  lean,
  lean4,
  less,
  liquid,
  lisp,
  lit,
  llvm,
  log,
  logo,
  lua,
  luau,
  make,
  makefile,
  markdown,
  markdownVue,
  marko,
  matlab,
  md,
  mdc,
  mdx,
  mediawiki,
  mermaid,
  mips,
  mipsasm,
  mjs,
  mmd,
  mojo,
  move,
  mts,
  nar,
  narrat,
  nextflow,
  nf,
  nginx,
  nim,
  nix,
  nu,
  nushell,
  objc,
  objectiveC,
  objectiveCpp,
  ocaml,
  pascal,
  perl,
  perl6,
  php,
  pkl,
  plsql,
  po,
  polar,
  postcss,
  pot,
  potx,
  powerquery,
  powershell,
  prisma,
  prolog,
  properties,
  proto,
  protobuf,
  ps,
  ps1,
  pug,
  puppet,
  purescript,
  py,
  python,
  ql,
  qml,
  qmldir,
  qss,
  r,
  racket,
  raku,
  razor,
  rb,
  reg,
  regex,
  regexp,
  rel,
  riscv,
  rosmsg,
  rs,
  rst,
  ruby,
  rust,
  sas,
  sass,
  scala,
  scheme,
  scss,
  sdbl,
  sh,
  shader,
  shaderlab,
  shell,
  shellscript,
  shellsession,
  smalltalk,
  solidity,
  soy,
  sparql,
  spl,
  splunk,
  sql,
  sshConfig,
  stata,
  styl,
  stylus,
  svelte,
  swift,
  systemVerilog,
  systemd,
  talon,
  talonscript,
  tasl,
  tcl,
  templ,
  terraform,
  tex,
  tf,
  tfvars,
  toml,
  ts,
  tsTags,
  tsp,
  tsv,
  tsx,
  turtle,
  twig,
  typ,
  typescript,
  typespec,
  typst,
  v,
  vala,
  vb,
  verilog,
  vhdl,
  vim,
  viml,
  vimscript,
  vue,
  vueDirectives,
  vueHtml,
  vueInterpolations,
  vueSfcStyleVariableInjection,
  vueVine,
  vy,
  vyper,
  wasm,
  wenyan,
  wgsl,
  wiki,
  wikitext,
  wit,
  wl,
  wolfram,
  xml,
  xsl,
  yaml,
  yml,
  zenscript,
  zig,
  zsh,
];
